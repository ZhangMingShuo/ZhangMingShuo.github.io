...
已知L的参数方程:\begin{cases}
              ...
              y=\psi(t)& \text{ }
            \end{cases} 
对弧长L的曲线积分 \int_{L}f(x,y)ds=\int^{\beta}_{\alpha}f[\phi(t),\psi(t)]\sqrt{\phi^{'2}(t)+\psi^{'2}(t)}dt(\alpha<\beta)
```
## 对坐标的曲线积分
```latex
...
## 高斯公式
```latex
设空间闭区域\Omega是由分片光滑的闭曲线\Sigma所围成，\\
P(x,y,z),Q(x,y,z),R(x,y,z)在\Omega上具有一阶连续偏导数,\\
